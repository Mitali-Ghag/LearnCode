...
exports.getMyEnrolledCourses = async (req, res) => {
  try {
    ...
    .filter((enrollment) => {
      if (enrollment.accessDurationWeeks === 0) return true; // Lifetime
...
exports.getStudentsEnrolledInCourse = async (req, res) => {
  try {
    ...
    if (!course) {
      return res
        .status(403)
        .json({ error: "Not authorized or course not found" });
    }
